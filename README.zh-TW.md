...
最多可以同時開八個編輯器視窗，每個視窗配一個自己的 ChatGPT 分頁。

在每個新視窗的命令選擇區執行**開一個與本視窗配對的分頁**。那個視窗會取一個空的連接埠
（`8765`–`8775`，跳過 `8767`–`8769`），並開出一個用 `?bridge_port=` 釘住該埠的分頁。
釘住的分頁只會連那一個視窗，別的視窗搶不走；子代理也各有自己的一段埠
（`8810 + 席次 * 8`），兩個視窗不會撞在一起。

面板會顯示本視窗握著哪個連接埠，方便分辨。

有兩件事要留意：

- **兩邊的擴充功能必須是同一版。** 舊版的瀏覽器擴充只看 `8765`，永遠找不到第二個視窗，
  而且失敗是無聲的。
- **一個帳號，一份用量上限。** 八個視窗各跑兩個子代理就是二十四個分頁同時對 ChatGPT 說話。
  實務上開兩三個視窗就差不多了。
